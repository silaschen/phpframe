...
Page({
  ...
  GetOrder: function (event, page) {
    if (inte1 == false) {
      ...
      var that = this;
      wx.request({
        url: App.data['API_URL'] + '?m=Travel&a=myorder&status=&sessionkey=' + wx.getStorageSync('sessionkey'),
        method: "GET",
        data: { "p": page },
        header: {
  ...
  GetOverOrder: function (event, page) {
    if (inte2 == false) {
      ...
      var that = this;
      wx.request({
        url: App.data['API_URL'] + '?m=Travel&a=myorder&status=2&sessionkey=' + wx.getStorageSync('sessionkey'),
        method: "GET",
        data: { "p": page },
        header: {
          'content-type': 'application/json'
  ...
  GetWaitOrder: function (event, page) {
    if (inte3 == false) {
      ...
      var that = this;
      wx.request({
        url: App.data['API_URL'] + '?m=Travel&a=myorder&status=1&sessionkey=' + wx.getStorageSync('sessionkey'),
        method: "GET",
        data: { "p": page },
        header: {
          'content-type': 'application/json'
